...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		return adventurerFunc(handPos, state);
    ...
    case smithy:
		return smithyFunc(handPos, state);
		
    case village:
		return villageFunc(handPos, state);
    ...
    case great_hall:
		return great_hallFunc(int handPos, struct gameState* state);
...
int smithyFunc(int currentPlayer, int handPos, struct gameState* state)
{
	int i;
      //+3 Cards
      for (i = 0; i < 2; i++)  //bug added to only add 2 cards instead of 3
	{
	  drawCard(currentPlayer, state);
	}
...
int adventurerFunc(int handPos, struct gameState* state, int temphand[], int z)
{
	...
	//bug added here to change from 2 cards to 4
   ...
   if (choice1 == 1)
	{
	  //+2 cards
	  drawCard(currentPlayer, state);
	  //bug added here. draw card function removed so only 1 card is drawn instead of 2.
	}
      else if (choice1 == 2)
	{
	  //+2 coins
...
int villageFunc(int handPos, int currentPlayer, struct gameState* state);
{
   //+1 Card
      ...
      //+2 Actions
      state->numActions = state->numActions + 1;  //bug added here to add only 1 action instead of 2
